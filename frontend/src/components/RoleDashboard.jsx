      ...
      {risks.length === 0 ? <div className="attention-empty"><CircleCheck aria-hidden="true" /> No significant delivery risks were detected from the currently available data.</div> : (
        <div className="attention-list">{risks.map((risk) => (
          <article key={risk.id} className={`severity-${risk.severity.toLowerCase()}`}>
            ...
            <div className="risk-actions"><button type="button" onClick={() => onViewIssue(risk)}><Eye aria-hidden="true" /> View Ticket</button><button type="button" onClick={() => onAsk(`Explain this delivery risk using the active dashboard evidence: ${risk.title}. Evidence: ${risk.evidence}. Separate observed facts, possible impact, missing context, and recommended action.`, { selected_metric: risk.metric, current_metric_value: risk.value })}><Sparkles aria-hidden="true" /> Ask Zara</button></div>
          </article>
        ))}</div>
      )}
    </section>
