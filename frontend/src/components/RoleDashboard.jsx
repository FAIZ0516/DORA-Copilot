  ...
  return (
    <section className="dashboard-filter-area" aria-label="Dashboard controls">
      <div className="dashboard-filter-bar">
        ...
        <label><span>Issue View</span><select value={issueView} onChange={(event) => onIssueViewChange(event.target.value)}><option value="all">All issues</option><option value="features">Features</option><option value="bugs">Bugs</option><option value="tests">Tests</option></select></label>
...
function RiskAttentionPanel({ payload, onViewIssue, onAsk }) {
      ...
      {risks.length === 0 ? <div className="attention-empty"><CircleCheck aria-hidden="true" /> No significant delivery risks were detected from the currently available data.</div> : (
        <div className="attention-list">{risks.map((risk) => (
          <article key={risk.id} className={`severity-${risk.severity.toLowerCase()}`}>
            ...
            <div className="risk-actions"><button type="button" onClick={() => onViewIssue(risk)}><Eye aria-hidden="true" /> View Issue</button><button type="button" onClick={() => onAsk(`Explain this delivery risk using the active dashboard evidence: ${risk.title}. Evidence: ${risk.evidence}. Separate observed facts, possible impact, missing context, and recommended action.`, { selected_metric: risk.metric, current_metric_value: risk.value })}><Sparkles aria-hidden="true" /> Ask Zara</button></div>
          </article>
        ))}</div>
      )}
    </section>
...
function IssueTable({ payload, filters, setFilters, onPage, onAsk }) {
  ...
  return (
    <section className="dashboard-issue-table" id="feature-issue-table" data-section="issues" aria-labelledby="work-table-title">
      <header><div><p>Operational detail</p><h3 id="work-table-title">Feature &amp; Issue Table</h3></div><span>{number(payload?.total || 0)} issues</span></header>
...
export default function RoleDashboard({ role, projectKey, projects = [], databaseConnected, onProjectChange, onAsk, disabled = false }) {
  ...
  return (
    <section className="role-dashboard" aria-label={`${roleLabel} dashboard`}>
      ...
      {payload?.empty && status !== "loading" && <div className="dashboard-empty-state"><BarChart3 /><strong>No Jira issues found in the selected scope.</strong><p>Clear a release, sprint, feature, or created-date filter and try again.</p><button type="button" onClick={resetFilters}>Clear filters</button></div>}

      {payload && !payload.empty && <>
        {isPortfolio ? <><div className="portfolio-kpi-row">{(payload.metric_cards || []).slice(0, 4).map((metric) => <article key={metric.key}><span>{metric.title}</span><strong>{number(metric.value, metric.key.includes("pct") ? "%" : "")}</strong><p>{metric.description}</p></article>)}</div><PortfolioView payload={payload} onSquad={viewSquad} onAsk={ask} /></> : <><PrimaryKpiRow payload={payload} onInfo={openMetric} onAsk={ask} /><DeliveryAnalytics payload={payload} /><RiskAttentionPanel payload={payload} onViewIssue={focusRisk} onAsk={ask} /><ProductivityOverview payload={payload} issues={visibleIssues} onAsk={ask} />{payload.release_information?.length > 0 && <section className="release-information"><header><h3>Release Information</h3><span>Rule-based source dates</span></header>{payload.release_information.map((release) => <article key={`${release.fixversion}-${release.release_date}`}><strong>{release.fixversion}</strong><span>Release date {release.release_date || "Unavailable"}</span><span>Plan {release.release_plan_start || "—"} → {release.release_plan_end || "—"}</span><span>Actual {release.release_actual_start || "—"} → {release.release_actual_end || "—"}</span></article>)}</section>}<IssueTable payload={visibleIssues} filters={issueFilters} setFilters={setIssueFilters} onPage={(page) => setIssueFilters((current) => ({ ...current, page }))} onAsk={ask} /></>}
