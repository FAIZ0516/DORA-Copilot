...
import Grainient, { ZARA_GRAINIENT_THEME } from "../../components/Grainient";
...
export default function ReportAssistantPanel({ report, selectedSection, busy, onSelectSection, onRefine }) {
  ...
  async function submit(event) {
    ...
    const updated = await onRefine(selectedSection, instruction.trim());
    if (updated) setInstruction("");
  }

  return (
    <aside className="report-assistant-panel" aria-label="Zara Report Assistant">
      <div className="report-assistant-grainient" aria-hidden="true"><Grainient {...ZARA_GRAINIENT_THEME} /></div>
      <header><ZaraAvatar decorative size={34} /><div><p><img src={zaraWordmark} alt="ZARA" /><strong>AI</strong></p><h3>Refine the report</h3><small>Report intelligence assistant</small></div></header>
